...
/*
  std::string stripped = StripPath(file);

std::cout << "In Md3.cpp: CreateTexture(): Texture file: " << stripped.c_str() << "\n";
 
  Texture* b = new Texture;   //TextureServer::Instance()->Get(stripped, "");
  b->ReadFile(file);
  b->SetFileName(file);
  b->BGRtoRGB(); 
  if (b)
  {
    b->CreateBinding(Texture::REGULAR);
  }
  return b;
*/
}
...
std::cout << "Got this line: '" << strLine << "' num splits: " << strs.size() << "\n";
		...
		if(!isdigit( strLine[0] ))
		{
			...
			//getline(fin, strLine);
                        // Need a new File function to get the remainder of the current line.
                        //strLine = f.TokenGetLineRemainder();
std::cout << "(ignoring this line: " << strLine.c_str() << ")\n";
		...
		//fin >> numOfFrames >> loopingFrames >> framesPerSecond;
                //f.GetIntToken(&numOfFrames);
                //f.GetIntToken(&loopingFrames);
                //f.GetIntToken(&framesPerSecond);
		...
		//fin >> strLine >> strLine;
                //strLine = f.TokenGetLineRemainder(); //GetDataLine(&strLine);
                //f.GetToken(&strLine);
                //f.GetToken(&strLine);
...
std::cout << "LOADING MODEL: " << strFileName << "\n";
	...
	// Open the MD3 file in binary
        ...
        filepluspath += strFileName; // TODO File root
	//m_FilePointer = fopen(filepluspath.c_str(), "rb");
...
//        std::cout << "Load header: loaded " << m_bytesRead << " bytes.\n";
//        std::cout << "Before Endianise, header: " << m_Header;
//        std::cout << "Loaded header, ID before Endianise: " << m_Header.version << "\n";
        m_Header.Endianise();
//        std::cout << "After Endianise(), header is now: " << m_Header;
