...
{
	if ((DWORD)user != req) return; // not the current request
...
DWORD WINAPI OpenURL(void *url)
{
	...
	MESS(30, WM_SETTEXT, 0, "");
	MESS(32, WM_SETTEXT, 0, "");
	newchan = BASS_StreamCreateURL((char*)url, 0, BASS_STREAM_BLOCK | BASS_STREAM_STATUS | BASS_STREAM_AUTOFREE | BASS_SAMPLE_FLOAT, StatusProc, (void*)r); // open URL
