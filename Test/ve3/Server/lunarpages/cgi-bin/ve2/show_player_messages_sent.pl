...
sub showsessions($$)
{
  ...
  my $dir = "../../ve2/results/";
  my $filename = $dir . "player-$player_id-messages-sent.csv";
  ...
  my $sql = "SELECT whensent, player.id, chat.id, msg FROM chat, player WHERE sender=$obj_id  AND whensent > DATE('2014-05-08') AND player.obj_id=chat.recip ORDER BY chat.id";
...
#  print "Query: $sql\n\n";
  ...
  while (my ($whensent,  $recip, $msgid, $msgtext) = $query->fetchrow_array)
  {
    my $decoded = pack('H*', "$msgtext");

    print "<tr>";
    print "<td>$whensent</td> <td>$player_id</td> <td>$recip</td> <td>$msgid</td> <td>$decoded</td> ";
    print "</tr>\n";

    print OUTFILE "$whensent, 1, $player_id, $recip, $msgid, \"$decoded\"\n";
  }
  print "</table>\n";
