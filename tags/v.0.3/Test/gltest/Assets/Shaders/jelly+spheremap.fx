// jelly+spheremap.fx
// Jelly wobble with Spherical Env Mapping
...
// Copied from Luna wave demo
static float a[2] = { 1.5f, 0.2f };
static float k[2] = { 4.0f, 8.0f };
    ...
    float2 tex0 : TEXCOORD0)
{
    ...
    // Make posL wobble - wave travels up y axis
    float wobble = 0.0f;
    for (int i = 0; i < 1; i++)
    {
        wobble += a[i] 
        * sin(k[i] * posL.y - gTime * w[i] + p[i]);
    }
    wobble = 1.0f + 0.1f * wobble;
    posL.x *= wobble;
    posL.z *= wobble;
    ...
    // Just an idea - doesn't fix the horrible artefacts :-(
    if (v.tex0.x < 0.01f)
    {
        v.tex0.x = 0.01f;
    }
