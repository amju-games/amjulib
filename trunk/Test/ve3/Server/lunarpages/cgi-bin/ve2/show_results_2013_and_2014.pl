...
##  THIS IS FOR 2013 EXPERIMENT ONWARDS!
## You get ALL the data for each participant, 
##  from Aug 2013 onwards!
#############################################
...
sub show_headings_forplayerfordate_test($$$$)
{
  ...
  my $sql = "SELECT DISTINCT rt.`key`  FROM `research_cogtest` as rct, research_testresult as rt, session as s, player as p where p.id=$player_id and rct.session_id=s.id and s.player_id=p.id and rct.id=rt.test_id and rct.test_type=$test_type and s.start >= '2013-08-23' order by rt.`key`";
...
sub showresultsforplayerbydate_testtype($$)
{
  ...
  my $sql = "SELECT count(DISTINCT s.start) FROM `research_cogtest` as rct, research_testresult as rt, session as s, player as p where p.id=$player_id and rct.session_id=s.id and s.player_id=p.id and rct.id=rt.test_id and s.start >= '2013-08-23' order by s.start";
  my $query = $dbh->prepare($sql) or die "Query prepare failed for this query: $sql\n";
  $query->execute;
  ...
  $sql = "SELECT DISTINCT s.start FROM `research_cogtest` as rct, research_testresult as rt, session as s, player as p where p.id=$player_id and rct.session_id=s.id and s.player_id=p.id and rct.id=rt.test_id and s.start >= '2013-08-23'  order by s.start";
  ...
  my $dir = "../../ve2/results/2013-and-2014/";
